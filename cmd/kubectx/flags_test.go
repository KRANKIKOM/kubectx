...
func Test_parseArgs_new(t *testing.T) {
	tests := []struct {
		...
		{name: "policy mode triggered by --mode",
			args: []string{"--mode=relaxed", "prod"},
			want: ReadonlyShellOp{Target: "prod", PolicyFlags: ReadonlyPolicyFlags{Mode: "relaxed"}}},
		{name: "policy mode triggered by --policy",
			args: []string{"--policy=ro.yaml", "prod"},
			want: ReadonlyShellOp{Target: "prod", PolicyFlags: ReadonlyPolicyFlags{PolicyFile: "ro.yaml"}}},
		{name: "policy mode triggered by --allow-exec",
			args: []string{"--allow-exec", "prod"},
			want: ReadonlyShellOp{Target: "prod", PolicyFlags: ReadonlyPolicyFlags{AllowExec: true}}},
		{name: "policy mode triggered by -n",
			args: []string{"-n", "dev", "prod"},
			want: ReadonlyShellOp{Target: "prod", PolicyFlags: ReadonlyPolicyFlags{Namespaces: []string{"dev"}}}},
